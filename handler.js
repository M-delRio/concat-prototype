...
const getOriginalVideo = async (originalVideoPath, jobData) => {
  ...
  console.log(
    `Retrieving original video from start bucket: ${filename}${inputType}`
  );
...
const mergeSegmentsIntoVideo = (mergedSegmentsPath) => {
  ...
  console.log(
    `Merging segments into single video, saving to ${mergedSegmentsPath}`
  );
  spawnSync(
    "/opt/ffmpeg/ffmpeg",
    [
...
const buildCompletedVideo = (videoPaths, jobData) => {
  if (!jobData.hasAudio) {
    console.log(
      `Video has no audio stream: completed path is now ${videoPaths.mergedPath}`
    );
...
const putCompletedVideoInBucket = async (jobData, completedVideoPath) => {
  ...
  console.log(
    "Completed file read into memory, unlinking file at path ",
    completedVideoPath
  );
